...
const char * const BITCOIN_CONF_FILENAME = "funbyte.conf";
const char * const BITCOIN_PID_FILENAME = "funbyted.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "funbyte" is a composite category enabling all FunByte-related debug output
            if(ptrCategory->count(string("funbyte"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FunByteCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FunByteCore
    // Mac: ~/Library/Application Support/FunByteCore
    // Unix: ~/.funbytecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FunByteCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FunByteCore";
#else
    // Unix
    return pathRet / ".funbytecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty funbyte.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override funbyte.conf
